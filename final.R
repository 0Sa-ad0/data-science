...
ds<-read.csv("C:/Users/SAAD/Downloads/DS-final-project/winequality-red.csv",header=TRUE,sep=",")
ds
str(ds)
breaks<-c(0, 3, 5, 7, 10)
...
ggplot(ds, aes(x = quality, fill = quality)) +
 ...
 pie <- ggplot(data = ds, aes(x = "", fill = quality)) +
  ...
  color_palette <- colorRampPalette(c("cyan", "blue"))(n = 100)
  ...
  ggplot(ds, aes(x = citric.acid, y = alcohol, size = fixed.acidity)) +
    ...
    labs(title = "Bubble Plot for Citric Acid and Alcohol",
         x = "Citric Acid",
         y = "Alcohol",
         size = "Fixed Acidity") +
